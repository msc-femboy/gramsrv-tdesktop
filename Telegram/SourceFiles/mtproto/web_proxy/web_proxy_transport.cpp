...
<main><h1>Telegram Web Proxy</h1><p id="state">Connecting to 100Gramm…</p><dl class="traffic"><div><dt>Sent through HTTPS</dt><dd><span id="up-total">0 B</span><small id="up-rate">0 B/s</small></dd></div><div><dt>Received through HTTPS</dt><dd><span id="down-total">0 B</span><small id="down-rate">0 B/s</small></dd></div></dl><p>Keep this tab open while using Telegram.</p><p class="note">Counts obfuscated carrier payload after successful requests; HTTPS overhead is not included.</p></main>
<script nonce="%2">
(()=>{
...
local.onclose=()=>{stopRtc();localClosed=true;state.textContent='100Gramm disconnected. Reopen the browser from Proxy Settings.';if(initialized)port.postMessage({t:'close'})};
local.onerror=()=>{state.textContent='Could not connect to 100Gramm.'};
...
port.onmessage=e=>{if(e.data instanceof ArrayBuffer){if(local.readyState===WebSocket.OPEN){if(local.bufferedAmount>localQueueLimit-e.data.byteLength){state.textContent='100Gramm is not consuming proxy data.';local.close();return}try{local.send(e.data)}catch(error){local.close()}}return}
